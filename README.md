...
> Course project for **Data Management for Data Science**, MSc in Data Science,
...
in Cypher. That is what makes the comparison between the three data models
...
**The forecast is equally good everywhere, and the ranking measures zone size
rather than difficulty.** In absolute terms North looks far and away the hardest
zone to predict: a mean absolute error of 281 MW against Sardinia's 12 MW. Measured
...
| --- | --- | --- | ---: | ---: |
| **Q3** | Correlated `AVG` per country, re-evaluated once per outer tuple | Materialized view + index on the join column | 9.7 s | **15 ms** |
